    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `WeatherEntity` (`location` TEXT NOT NULL, `jsonData` TEXT NOT NULL, PRIMARY KEY (`location`))');
...
class _$WeatherDao extends WeatherDao {
  ...
  Future<WeatherEntity?> getWeather(String location) async {
    return _queryAdapter.query(
        'SELECT * FROM WeatherEntity WHERE location = ?1',
  ...
  @override
  Future<List<WeatherEntity>> getAllWeatherData() async {
